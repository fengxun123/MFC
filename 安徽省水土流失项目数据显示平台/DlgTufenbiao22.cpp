...
MYSQL m_mysql22;			//数据库结构
MYSQL_RES* m_res22;         //查询的返回结果集
MYSQL_ROW m_row22;          //获取每一条记录
char query22[150]; //查询语句
...
BOOL CDlgTufenbiao22::InsertDB()
{
	...
	char* name = (char*)m_name22.GetBuffer();//将CString转换为char *
...
BOOL CDlgTufenbiao22::DeleteDB()
{
	...
	for (int i = iCount; i >= 0; i--)
	{
		if (m_list22.GetItemState(i, LVIS_SELECTED) == LVIS_SELECTED)
		{
			...
			sprintf(query22, "delete from tufen22 where 名称=\'%s\';", tname);
			if (mysql_query(&m_mysql22, query22))
			{
